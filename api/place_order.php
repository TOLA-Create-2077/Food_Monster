...
// 1. Include your optimized database connection setup
...
if (!empty($data)) {
    try {
        // Start a database transaction using your corrected $conn variable safely
        $conn->beginTransaction();

        // 3. Extract order request details explicitly mapped from Kotlin OrderRequest
        ...
        // Get the internal ID generated for this order transaction
        $orderId = $conn->lastInsertId();
        
        // Generate a clean tracking tracking code sequence (e.g., FM-000124)
        $generatedCode = "FM-" . str_pad($orderId, 6, "0", STR_PAD_LEFT);
        
        // Update the order row with the tracking sequence text
